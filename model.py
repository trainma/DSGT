...
class output_layer(nn.Module):
    ...
    def __init__(self,
        ...
        self.FC1 = nn.Linear(self.in_dim * self.history, self.hidden_dim,
                             bias=True)
    ...
    def forward(self, x):
        ...
        :return: (B, Tout, N)
        """
        batch_size = x.shape[0]  # x [32 4 307 64]

        x = x.permute(0, 2, 1, 3)  # B, N, Tin, Cin 32 307 4 64

        out1 = torch.relu(
            self.FC1(x.reshape(batch_size, self.num_of_vertices, -1)))
...
class STSGCN(nn.Module):
    ...
    def __init__(
        ...
        self.First_FC = nn.Linear(in_dim,
                                  first_layer_embedding_size,
                                  bias=True)
...
class TestModel2(nn.Module):
    ...
    def __init__(
        ...
        self.STSGCMS = nn.ModuleList()
        for _ in range(self.history - self.strides + 1):
            self.STSGCMS.append(
                STSGCM(
                    adj=self.adj,
                    in_dim=self.in_dim,
...
if __name__ == "__main__":
    print("Running")
    X = torch.randn(32, 12, 307,
                    64)  # [batch_size, time_step, num_of_vertices, in_dim]
