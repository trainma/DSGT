...
if __name__ == "__main__":
    ...
    rmse = masked_rmse(torch.from_numpy(predict),
                    torch.from_numpy(real), 0.0).item()
    print('mae: ', mae)
    print('rmse: ', rmse)
    ...
    for i in range(predict.shape[2]):
        ...
        rmse_station.append(masked_rmse(torch.from_numpy(
            predict[:, :, i, :]), torch.from_numpy(real[:, :, i, :])))
    ...
    for item in District:
        print('District: ', item)
        District_mae.append(np.mean(mae_station[item]))
        District_rmse.append(np.mean(rmse_station[item]))
        print('mae: ', np.mean(mae_station[item]))
        print('rmse: ', np.mean(rmse_station[item]))
    ...
    p = BJ_district_plot.plot(column='mae', scheme='BoxPlot', ax=ax, ec='black', lw=0.7, legend=True,
                            legend_kwds={'loc': 'center left',
                                        'bbox_to_anchor': (1, 0.5), 'interval': True, 'fmt': "{:.0f}"})
